...
function Page({ params: { id } }: Props) {
    ...
    const [durationError, setDurationError] = useState<DurationError>({ status: false, message: "" });
    const [isModified, setModified] = useState(false);

    async function handleEditMovie(data: FormType) {
    ...
    useEffect(() => {
        if (!isLoading) {
            setValue("name", data.movie.name);
            setValue("rating", data.movie.rating);
            setDuration(data.movie.duration + "h");
        }
    }, [isLoading]);
    ...
    return (
        <div id="defaultModal" aria-hidden="true" className={`flex backdrop-blur-sm overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full`}>
                ...
                <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <form onSubmit={handleSubmit(handleEditMovie)}>
                        ...
                        <div className='flex items-center'>
                            <button
                                type='submit'
                                className="text-white mr-4 inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">
                            ...
                            <Link
                                href="/"
                                className="inline-flex items-center  focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">
                                Cancel
                            </Link>
                        </div>
                    </form>
